...
public sealed class LauncherModificationDownloadCoordinatorTests
{
    ...
    [Fact]
    public void SuccessfulDownload_AutoDeleteCompletesWhenTheCatalogRescanMergesTheInstalledVersion()
    {
        StaTestRunner.Run(async () =>
        {
            StubPackageDownloadService downloadService = new(PackageDownloadResult.Succeeded());
            ...
            // Reconciling rescans disk and merges the freshly installed version back into the card being cleaned up,
            // which is what an in-flight enumerator over that same version list cannot survive.
            var catalog = new FakeLauncherContentCatalog
            {
                Data = data,
                LocalDataUpdateHandler = () =>
                    data.AddOrUpdate(CreateVersion("Shockwave", "3.0", installed: true))
            };
            LauncherModificationDownloadCoordinator coordinator = CreateCoordinator(
                downloadService,
                activityService,
            ...
            LauncherContentKey[] expectedUninstalls = [oldest.ContentKey, older.ContentKey];
            catalog.UninstalledVersions.Should().BeEquivalentTo(expectedUninstalls);
