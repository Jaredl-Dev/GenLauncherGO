...
public sealed class LauncherTileActionServiceTests
{
    ...
    [Fact]
    public void DeleteVersionForModWithAnotherInstalledVersion_UninstallsOnlyTheSelectedVersion()
    {
        var catalog = new FakeLauncherContentCatalog();
        LauncherTileActionService service = new(catalog);
        LauncherContentVersion selectedVersion = new()
        ...
        LauncherContentVersion retainedVersion = new()
        {
            Installation = new LauncherContentInstallation { Installed = true },
            Name = "Operation Firestorm",
            Version = "0.3.17.2",
        ...
        ModificationVersionSelection versionSelection = new(
            selectedVersion,
            CreateViewModel(selectedVersion, additionalVersion: retainedVersion));
        ...
        catalog.UninstalledVersions.Should().ContainSingle().Which.Should().Match<LauncherContentKey>(contentKey =>
            contentKey.Name == "Operation Firestorm" &&
            contentKey.Version == "0.3.17.1" &&
            contentKey.ContentType == ModificationType.Mod);
        catalog.DiscardedContents.Should().BeEmpty();
    }

    [Fact]
    public void DeleteVersionForModWhoseOtherVersionIsNotInstalled_DiscardsContent()
    {
        var catalog = new FakeLauncherContentCatalog();
        LauncherTileActionService service = new(catalog);
        LauncherContentVersion selectedVersion = new()
        ...
        LauncherContentVersion availableVersion = new()
        {
            Name = "Operation Firestorm",
            Version = "0.3.17.3",
            ModificationType = ModificationType.Mod
        ...
        ModificationVersionSelection versionSelection = new(
            selectedVersion,
            CreateViewModel(selectedVersion, additionalVersion: availableVersion));
    ...
    [Fact]
    public void VersionSelection_RejectsChildContentFromADifferentTile()
    {
        LauncherContentVersion selectedVersion = new()
        {
            Installation = new LauncherContentInstallation
        ...
        Action createSelection = () => new ModificationVersionSelection(
            selectedVersion,
            CreateViewModel(selectedVersion, "Shockwave"));

        createSelection.Should().Throw<ArgumentException>();
    }
    ...
    private static ModificationViewModel CreateViewModel(
        ...
        LauncherContentVersion? additionalVersion = null)
    {
        ...
        LauncherContent content = new(contentVersion);
        if (additionalVersion is not null)
        {
            content.AddOrMergeVersion(additionalVersion);
        }
