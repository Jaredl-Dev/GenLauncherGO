...
public sealed partial class LauncherWindowWorkflowCoordinatorTests
{
    ...
    [Fact]
    public void OpenOptionsAsync_WhenTheSettingsGameSwitchIsApplied_RestoresTheTargetGameScrollPosition()
    {
        StaTestRunner.Run(async () =>
        {
            using ApplicationThemeScope themeScope = new();
            var preferencesService = new RecordingLauncherPreferencesService(new LauncherPreferences
            {
                ...
                {
                    Generals = new LauncherGamePreferences { ModsListVerticalOffset = 35 }
                }
            });
            FakeLauncherContentCatalog catalog = CreateCatalog();
            ...
            {
                fixture.AddTile(CreateTile(TestLauncherContent.From(TestLauncherContent.Version(
                    $"Scrollable mod {index}",
                    installed: true))));
            }

            fixture.Owner.Width = 320;
            fixture.Owner.Height = 140;
            fixture.Owner.Content = fixture.ModsList;
            fixture.Owner.Show();
            fixture.Owner.UpdateLayout();
            LauncherDragDropController.RestoreVerticalScrollOffset(fixture.ModsList, 80);
            double outgoingOffset = LauncherDragDropController.GetVerticalScrollOffset(fixture.ModsList);
            double restoredOffset = -1;
            fixture.ViewModel.PropertyChanged += (_, args) =>
            {
                if (args.PropertyName == nameof(MainWindowViewModel.MainControlsEnabled) &&
                    fixture.ViewModel.MainControlsEnabled)
                {
                    restoredOffset = LauncherDragDropController.GetVerticalScrollOffset(fixture.ModsList);
                }
            };
            ...
            preferencesService.Current.Games.ZeroHour.ModsListVerticalOffset.Should().BeApproximately(
                outgoingOffset,
                0.1);
            preferencesService.Current.Games.Generals.ModsListVerticalOffset.Should().Be(35);
            restoredOffset.Should().BeApproximately(35, 0.1);
