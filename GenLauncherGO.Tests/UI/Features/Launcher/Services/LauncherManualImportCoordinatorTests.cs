...
public sealed class LauncherManualImportCoordinatorTests
{
    ...
    /// <summary>
    ///     Choosing the folder answer runs the folder picker instead of the file picker, and the chosen folder is the
    ///     single import source.
    /// </summary>
    [Fact]
    public void ImportAsync_WhenTheUserChoosesAFolder_ImportsThatFolder()
    {
        StaTestRunner.Run(async () =>
        {
            ...
            ILauncherFilePicker filePicker = new StubLauncherFilePicker
            {
                ManualPackageFilesResult = [@"C:\Downloads\ignored.zip"],
                ManualContentFolderResult = @"C:\Downloads\Shockwave"
            };
            ILauncherDialogService dialogService = Substitute.For<ILauncherDialogService>();
            ...
            dialogService.ShowManualModificationImportAsync(
                    Arg.Any<IReadOnlyList<string>>(),
                    Arg.Any<Window?>())
                .Returns(Task.FromResult<ManualModificationDialogResult?>(
            ...
            importer.Received(1).Import(
                Arg.Is<IReadOnlyList<string>>(sources =>
                    sources.Count == 1 && sources[0] == @"C:\Downloads\Shockwave"),
    ...
    /// <summary>
    ///     Cancelling the folder picker cancels the import rather than falling back to the file picker.
    /// </summary>
    [Fact]
    public void ImportAsync_WhenTheFolderPickerIsCanceled_ReturnsNull()
    {
        StaTestRunner.Run(async () =>
        {
            ILauncherFilePicker filePicker = new StubLauncherFilePicker
            ...
            dialogService.ShowInfoActionAsync(
                    ...
                    Arg.Any<Window?>())
                .Returns(Task.FromResult(true));
            LauncherManualImportCoordinator coordinator = CreateCoordinator(filePicker, dialogService);
