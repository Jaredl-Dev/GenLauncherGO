...
public sealed class LauncherWindowListControllerTests
{
    ...
    /// <summary>
    ///     Deleting the last installed modifications rebuilds the mods list while the selection still names the
    ///     positions the tiles used to occupy, so the change lists Avalonia hands the handler can no longer be read.
    ///     The stale notification has to be ignored rather than surfaced to the user as a crash.
    /// </summary>
    [Fact]
    public void HandleModsListSelectionChangedAsync_WhenTheSelectionCanNoLongerBeResolved_IsIgnored()
    {
        StaTestRunner.Run(async () =>
        {
            var catalog = new FakeLauncherContentCatalog();
            ControllerFixture fixture = CreateFixture(catalog);
            var stale = new SelectionChangedEventArgs(
                SelectingItemsControl.SelectionChangedEvent,
                new StaleSelectionList(),
                Array.Empty<ModificationViewModel>())
            {
                Source = fixture.ModsList
            };

            Func<Task> act = () => fixture.Content.HandleModsListSelectionChangedAsync(stale);
    ...
    [Fact]
    public void HandleChildContentListSelectionChanged_WhenTheSelectionCanNoLongerBeResolved_IsIgnored()
    {
        StaTestRunner.Run(() =>
        {
            var catalog = new FakeLauncherContentCatalog();
            ...
            var stale = new SelectionChangedEventArgs(
                ...
                new StaleSelectionList())
            {
                Source = fixture.PatchesList
            };

            Action act = () => fixture.Content.HandleChildContentListSelectionChanged(stale);
    ...
    public void HandleChildContentListSelectionChanged_FromPatchesList_ReplacesTheSelectedPatch()
    {
        StaTestRunner.Run(() =>
        {
            ...
            fixture.PatchesList.Name = "RenamedPatchesControl";
    ...
    /// <summary>
    ///     Stands in for the live selection view Avalonia hands a selection-changed handler once the list it indexes
    ///     into has been rebuilt beneath it: it reports a selection but throws while reading one.
    /// </summary>
    private sealed class StaleSelectionList : IList
    {
