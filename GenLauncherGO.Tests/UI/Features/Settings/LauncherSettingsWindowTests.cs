...
public sealed class LauncherSettingsWindowTests
{
    ...
    [Fact]
    public void DiagnosticLoggingToggle_WhenChecked_PersistsThePreference()
    {
        StaTestRunner.Run(() =>
        {
            using ApplicationThemeScope themeScope = new();
            ...
            {
                scenario.Window.Show();
                CheckBox toggle = scenario.Window.FindControl<CheckBox>("DiagnosticLoggingCheckBox")!;

                toggle.IsChecked.Should().BeFalse();
                toggle.IsChecked = true;

                scenario.PreferencesService.Current.Shared.EnableDiagnosticLogging.Should().BeTrue();
            }
            finally
            {
                scenario.Window.Close();
