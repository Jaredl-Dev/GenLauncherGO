...
public sealed class WindowsLauncherHostEnvironmentServiceTests
{
    ...
    public async Task TryAcquireSingleInstance_ReturnsAcquiredGuardWhenNameIsReleasedBeforeRetryAsync()
    {
        ...
        try
        {
            mutexAcquired.Wait(TestTimeouts.Wait, TestContext.Current.CancellationToken).Should().BeTrue();
            ...
            retryStarted.Wait(TestTimeouts.Wait, TestContext.Current.CancellationToken).Should().BeTrue();

            releaseMutex.Set();
            ownerThread.Join();
            allowRetry.Set();
            using ILauncherSingleInstanceGuard guard =
                await acquisition.WaitAsync(TestTimeouts.Wait, TestContext.Current.CancellationToken);
