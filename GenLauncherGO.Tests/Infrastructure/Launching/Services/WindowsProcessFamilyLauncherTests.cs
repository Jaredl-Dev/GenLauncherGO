...
public sealed class WindowsProcessFamilyLauncherTests
{
    ...
    [Fact]
    public void ProcessFamilyTracker_RootExecutableNameDoesNotExposeDirectory()
    {
        string executablePath = Path.Combine("C:", "Games", "Zero Hour", "generalszh.exe");
        WindowsProcessFamilyLauncher.ProcessFamilyTracker tracker = CreateTracker(
            10,
            captureProcessSnapshot: Array.Empty<WindowsProcessFamilyLauncher.ProcessSnapshotEntry>,
            rootExecutableName: executablePath);
    ...
    public async Task StartAsync_ReportsHowLongTheProcessFamilyRanAsync()
    {
        ...
        IProcessFamilyLaunchOperation operation = await launcher.StartAsync(
            ...
            directory.Path,
            CancellationToken.None);
        TimeSpan runningDuration = await operation.Completion.WaitAsync(TimeSpan.FromSeconds(25), TestContext.Current.CancellationToken);
    ...
    public async Task StartAsync_DoesNotReportExecutableNameChangesWhileTheSameProcessRunsAsync()
    {
        ...
        await operation.Completion.WaitAsync(TimeSpan.FromSeconds(25), TestContext.Current.CancellationToken);
