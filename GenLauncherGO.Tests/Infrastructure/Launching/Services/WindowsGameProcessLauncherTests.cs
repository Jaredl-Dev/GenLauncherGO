...
public sealed class WindowsGameProcessLauncherTests
{
    ...
    [Fact]
    public async Task StartAsync_ShortGameLaunchWarningDoesNotExposeGameDirectoryAsync()
    {
        using var directory = new TestDirectory();
        directory.CreateFile("custom.exe", string.Empty);
        var processLauncher = new RecordingProcessFamilyLauncher
        {
            RunningDuration = TimeSpan.FromSeconds(1)
        };
        RecordingLogger<WindowsGameProcessLauncher> logger = new();
        WindowsGameProcessLauncher launcher = new(processLauncher, logger);
        ...
        logger.Entries.Should().ContainSingle(entry =>
            ...
            entry.Message.Contains("custom.exe", StringComparison.Ordinal) &&
            !entry.Message.Contains(directory.Path, StringComparison.OrdinalIgnoreCase));
    }
