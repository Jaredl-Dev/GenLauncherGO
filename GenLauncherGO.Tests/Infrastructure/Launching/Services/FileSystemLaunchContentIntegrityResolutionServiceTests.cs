...
public sealed class FileSystemLaunchContentIntegrityResolutionServiceTests
{
    ...
    public async Task InitializeUntrackedManagedContentAsync_CapturesCacheWhenExpectedAssetsMatchAsync(
        ...
        string expectedImageFileName)
    {
        ...
        bool initialized = await service.InitializeUntrackedManagedContentAsync(
    ...
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WhenCacheAlsoCarriesAnotherIssue_LeavesItForResolutionAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = CreateVersion(ContentSourceKind.ManagedSingleFile);
    ...
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WhenCacheContentsDoNotMatch_ReportsNoInitializationAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = CreateVersion(ContentSourceKind.ManagedSingleFile);
    ...
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WithUntrackedSingleFilePackage_LeavesItForResolutionAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = CreateVersion(ContentSourceKind.ManagedSingleFile);
    ...
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WhenS3PackageMatchesItsManifest_AdoptsItAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = TestLauncherContent.S3Version(version: "1.2");
        string packageRoot = Path.Combine(paths.ModsDirectory, "ShockWave", "1.2");
        await WriteInstalledPackageFilesAsync(packageRoot);
        ContentIntegrityTarget packageTarget = CreateTarget(
            "package",
            packageRoot,
        ...
        manifestReader.Enqueue(CreateManifest());
        RecordingContentIntegrityService integrityService = CreateIntegrityService();
        integrityService.CaptureIfMatchesExpectedFileSetResult = true;
        FileSystemLaunchContentIntegrityResolutionService service = CreateService(
            integrityService,
        ...
        initialized.Should().BeTrue();
        // The manifest names Data/file.big; the installed package holds it under the converted .gib name.
        integrityService.ConditionalSnapshotRequests.Should().ContainSingle(request =>
            request.Target == packageTarget &&
            request.ExpectedRelativePaths.SetEquals(new[] { "Data/file.gib", "Data/readme.txt" }));
    }

    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WhenAnS3PackageFileHasTheWrongSize_LeavesItForResolutionAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = TestLauncherContent.S3Version(version: "1.2");
        string packageRoot = Path.Combine(paths.ModsDirectory, "ShockWave", "1.2");
        await WriteInstalledPackageFilesAsync(packageRoot, packageFileContents: "0123456789-longer");
        ContentIntegrityTarget packageTarget = CreateTarget(
            "package",
            packageRoot,
            ContentSourceKind.ManagedS3);
    ...
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WhenAnS3PackageFileHasTheWrongHash_LeavesItForResolutionAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = TestLauncherContent.S3Version(version: "1.2");
        ...
        FileSystemLaunchContentIntegrityResolutionService service = CreateService(
            integrityService,
            manifestReader: manifestReader,
            fileHashService: new StubFileHashService
            {
                HashForPath = _ => StubFileHashService.MismatchedHash
            });
    ...
    /// <summary>
    ///     Proves an unreachable manifest leaves the package for the review dialog instead of blocking the launch,
    ///     which is what an offline session does to content that was moved in from another launcher.
    /// </summary>
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WhenTheManifestCannotBeRead_LeavesItForResolutionAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = TestLauncherContent.S3Version(version: "1.2");
        ...
        FileSystemLaunchContentIntegrityResolutionService service = CreateService(
            integrityService,
            manifestReader: new ThrowingS3ObjectManifestReader());
    ...
    public async Task ResolveAsync_RefreshesManagedCacheAndPreservesIgnoredFilesAsync()
    {
        ...
        await File.WriteAllTextAsync(ignoredFilePath, "ignored", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(Path.Combine(staleNestedDirectory, "nested.txt"), "nested", TestContext.Current.CancellationToken);
    ...
    public async Task ResolveAsync_RepairsManagedSingleFilePackageAndReportsProgressAsync()
    {
        ...
        (DownloadFileMetadata Metadata, PackageUpdatePathSet Paths) updateRequest =
            singleFilePackageUpdater.Requests.Should().ContainSingle().Which;
        updateRequest.Metadata.DownloadUri.Should().Be(new Uri("https://www.dropbox.com/s/package/file.zip?dl=1"));
    ...
    /// <summary>
    ///     An import whose name and version match a published package still resolves to managed content, so it is
    ///     snapshotted exactly as a download would have been.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    ///     Content with no published package behind it is not snapshot-tracked at all, so importing it records the
    ///     classification and nothing else.
    /// </summary>
    [Fact]
    public async Task RegisterManualImportAsync_WithUnmanagedVersion_CapturesNoSnapshotsAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = CreateVersion(
        ...
        version.Installation.ContentSourceKind.Should().Be(ContentSourceKind.Manual);
        catalog.SaveCount.Should().Be(1);
        integrityService.CapturedTargets.Should().BeEmpty();
        integrityService.ConditionalSnapshotRequests.Should().BeEmpty();
    }
    ...
    public async Task CaptureManagedInstallSnapshotAsync_RefreshesMismatchedThemedImageCacheAsync()
    {
        ...
        await File.WriteAllTextAsync(staleFilePath, "stale", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(inactiveImagePath, "inactive", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(themeCachePath, "theme", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(backgroundPath, "old background", TestContext.Current.CancellationToken);
        RecordingContentIntegrityService integrityService = CreateIntegrityService();
        var assetDownloader = new RecordingRemoteAssetDownloader
        {
        ...
        (await File.ReadAllTextAsync(backgroundPath, TestContext.Current.CancellationToken)).Should().Be("downloaded");
    ...
    /// <summary>
    ///     Adoption only ever applies to content with a trusted remote source, so an unmanaged target that reports a
    ///     problem is never snapshotted on its way past the review.
    /// </summary>
    [Fact]
    public async Task InitializeUntrackedManagedContentAsync_WithUnmanagedTarget_LeavesItForResolutionAsync()
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
    ...
    private static FileSystemLaunchContentIntegrityResolutionService CreateService(
        ...
        IDownloadFileMetadataReader? metadataReader = null,
        IFileHashService? fileHashService = null)
    {
        var packageSourceResolver = new ManagedPackageSourceResolver(
            metadataReader ?? new StubDownloadFileMetadataReader("package.zip", null),
    ...
    /// <summary>
    ///     Builds the manifest the installed-package fixtures are written to match.
    /// </summary>
    private static RemoteFileManifestEntry[] CreateManifest()
    {
        return
        [
            new RemoteFileManifestEntry("Data/file.big", StubFileHashService.MatchingHash, 10),
            new RemoteFileManifestEntry("Data/readme.txt", StubFileHashService.MatchingHash, 5)
        ];
    }

    /// <summary>
    ///     Writes the package folder a completed S3 install leaves behind, with <c>.big</c> already converted to the
    ///     inert <c>.gib</c> name.
    /// </summary>
    private static async Task WriteInstalledPackageFilesAsync(
        string packageRoot,
        string packageFileContents = "0123456789")
    {
        Directory.CreateDirectory(Path.Combine(packageRoot, "Data"));
        await File.WriteAllTextAsync(
            Path.Combine(packageRoot, "Data", "file.gib"),
            packageFileContents,
            TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(
            Path.Combine(packageRoot, "Data", "readme.txt"),
            "01234",
            TestContext.Current.CancellationToken);
    }
    ...
    /// <summary>
    ///     Fails every manifest read, standing in for a session that cannot reach the remote source.
    /// </summary>
    private sealed class ThrowingS3ObjectManifestReader : IS3ObjectManifestReader
    {
        public Task<IReadOnlyList<RemoteFileManifestEntry>> ReadManifestAsync(
            S3ObjectManifestRequest request,
            CancellationToken cancellationToken)
        {
            throw new IOException("The remote manifest is unavailable.");
        }
    }
