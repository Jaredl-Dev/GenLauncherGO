...
public sealed class ContentIntegrityScannerTests
{
    ...
    public async Task ScanAsync_MissingRootDirectory_ReturnsEmptyResultAsync()
    {
        ...
        ContentIntegrityScanResult scan = await ContentIntegrityScanner.ScanAsync(target, true, CancellationToken.None);
    ...
    public async Task ScanAsync_CancelledToken_ThrowsAsync()
    {
        ...
        Func<Task> scan = () => ContentIntegrityScanner.ScanAsync(target, true, cancellation.Token);
    ...
    /// <summary>
    ///     A safety-only scan is what unmanaged content gets, so it has to keep finding the entries deployment must
    ///     not follow while never reading a file it only needs to prove is readable.
    /// </summary>
    [Fact]
    public async Task ScanAsync_WithoutFileHashes_StillReportsUnreadableEntriesAndLeavesHashesEmptyAsync()
    {
        using TestDirectory directory = new();
        string content = directory.CreateDirectory("content");
        directory.CreateFile("content/readable.bin", "readable");
        string lockedPath = directory.CreateFile("content/locked.bin", "locked");
        ContentIntegrityTarget target = CreateTarget(content);
        using FileStream exclusiveHandle = new(lockedPath, FileMode.Open, FileAccess.Read, FileShare.None);

        ContentIntegrityScanResult scan = await ContentIntegrityScanner.ScanAsync(target, false, CancellationToken.None);

        scan.Errors.Should().ContainSingle().Which.RelativePath.Should().Be("locked.bin");
        ContentIntegrityScannedFile scannedFile = scan.Files.Should().ContainSingle().Which.Value;
        scannedFile.RelativePath.Should().Be("readable.bin");
        scannedFile.Size.Should().Be("readable".Length);
        scannedFile.Sha256.Should().BeEmpty();
    }
