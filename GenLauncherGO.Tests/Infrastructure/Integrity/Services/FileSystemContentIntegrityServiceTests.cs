...
public sealed class FileSystemContentIntegrityServiceTests
{
    ...
    public async Task VerifyAsync_UnmanagedContentThatChanged_ReportsNoIssuesAsync(ContentSourceKind sourceKind)
    {
        using TestDirectory directory = new();
        LauncherPaths paths = CreatePaths(directory);
        string content = directory.CreateDirectory("content");
    ...
    /// <summary>
    ///     Unmanaged content is never snapshot-tracked, so a first launch has nothing to migrate and nothing to ask
    ///     the user about.
    /// </summary>
    [Fact]
    public async Task VerifyAsync_UnmanagedContentWithoutSnapshot_ReportsNoIssuesAsync()
    {
        using TestDirectory directory = new();
        LauncherPaths paths = CreatePaths(directory);
        string content = directory.CreateDirectory("content");
    ...
    public async Task CaptureSnapshotAsync_OrdersEntriesIndependentlyOfEnumerationAsync()
    {
        ...
        LauncherPaths paths = CreatePaths(directory);
        string content = directory.CreateDirectory("content");
        await File.WriteAllTextAsync(Path.Combine(content, "zeta.txt"), "z", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(Path.Combine(content, "alpha.txt"), "a", TestContext.Current.CancellationToken);
    ...
    public async Task VerifyAsync_PreservesIgnoredInactiveCacheFileAsync()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(content, "inactive.png"), "inactive", TestContext.Current.CancellationToken);
        FileSystemContentIntegrityService service = CreateService();
    ...
    public async Task CaptureSnapshotIf_MatchesExpectedFileSetAsyncCapturesExistingManagedCacheWithoutMutationAsync()
    {
        ...
        await File.WriteAllTextAsync(activePath, "active", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(inactivePath, "inactive", TestContext.Current.CancellationToken);
        FileSystemContentIntegrityService service = CreateService();
        ContentIntegrityTarget target = new(
            "target",
            "Target",
        ...
        report.HasIssues.Should().BeFalse();
        (await File.ReadAllTextAsync(activePath, TestContext.Current.CancellationToken)).Should().Be("active");
        (await File.ReadAllTextAsync(inactivePath, TestContext.Current.CancellationToken)).Should().Be("inactive");
    }
    ...
    [Fact]
    public void PruneSnapshots_WithoutASnapshotDirectory_DeletesNothing()
    {
        using TestDirectory directory = new();
        LauncherPaths paths = CreatePaths(directory);
        FileSystemContentIntegrityService service = CreateService();

        int deletedCount = service.PruneSnapshots(paths, new HashSet<string>(StringComparer.Ordinal));
