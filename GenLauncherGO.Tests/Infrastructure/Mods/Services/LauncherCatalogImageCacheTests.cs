...
public sealed class LauncherCatalogImageCacheTests
{
    ...
    public async Task CacheModificationImagesAsync_CachesPublishedPaletteAndBackgroundArtworkAsync()
    {
        ...
        themeCache.Entries[modification.ContentKey].Should().BeSameAs(theme);
        assetDownloader.Calls.Should().ContainSingle(call =>
    ...
    public async Task CacheAdvertisingImagesAsync_DeletesStaleImagesWhenImageCountChangesAsync()
    {
        ...
        await File.WriteAllTextAsync(staleImagePath, "stale", TestContext.Current.CancellationToken);
        var assetDownloader = new RecordingRemoteAssetDownloader();
        LauncherCatalogImageCache cache = CreateCache(assetDownloader);
    ...
    public async Task CacheAdvertisingImagesAsync_KeepsExistingImagesWhenImageCountMatchesAsync()
    {
        ...
        await File.WriteAllTextAsync(firstExistingImagePath, "existing", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(secondExistingImagePath, "existing", TestContext.Current.CancellationToken);
        var assetDownloader = new RecordingRemoteAssetDownloader();
        LauncherCatalogImageCache cache = CreateCache(assetDownloader);
        var advertisingData = new RemoteAdvertisingReference(
            "Featured Mod",
    ...
    public async Task CacheAdvertisingImagesAsync_DoesNotMutateThroughLinkedImageDirectoryAsync()
    {
        ...
        await File.WriteAllTextAsync(firstOutsideImage, "first", TestContext.Current.CancellationToken);
        await File.WriteAllTextAsync(secondOutsideImage, "second", TestContext.Current.CancellationToken);
        ReparsePointTestSupport.CreateDirectoryJunction(
        ...
        assetDownloader.Calls.Should().BeEmpty();
        (await File.ReadAllTextAsync(firstOutsideImage, TestContext.Current.CancellationToken)).Should().Be("first");
        (await File.ReadAllTextAsync(secondOutsideImage, TestContext.Current.CancellationToken)).Should().Be("second");
    }
