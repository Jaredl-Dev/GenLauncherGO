...
public sealed class FileSystemModificationThemeCacheTests
{
    ...
    public void CachedPalette_SurvivesAReadBack()
    {
        ...
        LauncherContentKey contentKey = CreateKey("Contra", "1.0");
        cache.Save(contentKey, theme);

        LauncherContentTheme? loaded = cache.Load(contentKey);
    ...
    public void UncachedPalette_LoadsAsNothing()
    {
        ...
        cache.Load(CreateKey("Contra", "1.0")).Should().BeNull();
    }
    ...
    public void CachedPalette_LivesInTheModificationImageCacheFolder()
    {
        ...
        cache.Save(
            CreateKey("Contra", "1.0"),
            new LauncherContentTheme { GenLauncherActiveColor = "#baff0c" });
    ...
    public void Save_KeepsNothingForBlankContentIdentity(string modificationName, string version)
    {
        ...
        LauncherContentKey contentKey = CreateKey(modificationName, version);
        cache.Save(contentKey, new LauncherContentTheme { GenLauncherActiveColor = "#baff0c" });

        cache.Load(contentKey).Should().BeNull();
    ...
    public void Load_ReturnsNothingForAnUnreadableCachedDocument(string cachedDocument)
    {
        ...
        LauncherContentTheme? loaded = cache.Load(CreateKey("Contra", "1.0"));
    ...
    private static LauncherContentKey CreateKey(string name, string version)
    {
        return new LauncherContentKey(ModificationType.Mod, string.Empty, name, version);
    }
}
