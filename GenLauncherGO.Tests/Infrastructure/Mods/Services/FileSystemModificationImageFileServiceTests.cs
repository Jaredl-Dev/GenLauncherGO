...
public sealed class FileSystemModificationImageFileServiceTests
{
    ...
    public void FindExistingImageFilePath_FindsCachedImageWhateverItsExtension()
    {
        ...
        LauncherPaths paths = TestLauncherPaths.Create(directory.CreateDirectory("Game"));
        ...
        string? existingImagePath = service.FindExistingImageFilePath(ModificationType.Mod, "ShockWave", "1.2");
    ...
    public void TryDeleteImage_AlsoRemovesTheImageOfAVersionThatOnlyExtendsTheRequestedName()
    {
        ...
        bool deleted = service.TryDeleteImage(ModificationType.Mod, "ShockWave", "1.2");
    ...
    public void FindExistingImageFilePath_ReturnsNullWhenNoCachedImageMatches()
    {
        ...
        string? existingImagePath = service.FindExistingImageFilePath(ModificationType.Mod, "ShockWave", "3.0");
    ...
    public async Task ReplaceImageAsync_AlsoRemovesTheImageOfAVersionThatOnlyExtendsTheRequestedNameAsync()
    {
        ...
        Directory.CreateDirectory(imageDirectory);
        string siblingImagePath = Path.Combine(imageDirectory, "1.2.5.png");
        await File.WriteAllTextAsync(siblingImagePath, "sibling", TestContext.Current.CancellationToken);
        ...
        File.Exists(siblingImagePath).Should().BeFalse();
        (await File.ReadAllTextAsync(destinationPath, TestContext.Current.CancellationToken)).Should().Be("new");
    }
    ...
    {
        using TestDirectory directory = new();
        FileSystemModificationImageFileService service = CreateService(TestLauncherPaths.Create(directory.CreateDirectory("Game")));

        string? existingImagePath = service.FindExistingImageFilePath(ModificationType.Mod, "Missing", "1.2");
    ...
    public void CountImageFiles_ReturnsZeroForMissingDirectory()
    {
        ...
        int count = service.CountImageFiles(ModificationType.Mod, "Missing");
    ...
    public void CountImageFiles_ReturnsImageFileCount()
    {
        ...
        int count = service.CountImageFiles(ModificationType.Mod, "ShockWave");

        count.Should().Be(2);
    }

    [Fact]
    public void CountImageFiles_AdvertisingUsesTheCatalogCacheName()
    {
        using TestDirectory directory = new();
        LauncherPaths paths = TestLauncherPaths.Create(directory.CreateDirectory("Game"));
        string imageDirectory = paths.GetModificationImagesDirectory("Do you like GenLauncher");
        Directory.CreateDirectory(imageDirectory);
        File.WriteAllText(Path.Combine(imageDirectory, "0.jpg"), "image");
        FileSystemModificationImageFileService service = CreateService(paths);

        int count = service.CountImageFiles(
    ...
    public void TryDeleteImage_RejectsUnsafeCacheIdentity()
    {
        ...
        bool deleted = service.TryDeleteImage(ModificationType.Mod, "..", "victim");
    ...
    public void TryDeleteImage_ReturnsTrueWhenFileDoesNotExist()
    {
        ...
        bool deleted = service.TryDeleteImage(ModificationType.Mod, "ShockWave", "missing");
    ...
    public void FindExistingImageFilePath_RejectsLinkedImageDirectory()
    {
        ...
        Action act = () => service.FindExistingImageFilePath(ModificationType.Mod, "ShockWave", "1.2");
    ...
    public async Task ReplaceImageAsync_DoesNotFollowLinkedImageDirectoryAsync()
    {
        ...
        await File.WriteAllTextAsync(outsideImagePath, "outside", TestContext.Current.CancellationToken);
        ReparsePointTestSupport.CreateDirectoryJunction(
            paths.GetModificationImagesDirectory("ShockWave"),
            outsideDirectory);
        string sourceImagePath = directory.CreateFile("selected.png", "new");
        ...
        (await File.ReadAllTextAsync(outsideImagePath, TestContext.Current.CancellationToken)).Should().Be("outside");
    ...
    public async Task ReplaceImageAsync_DeletesStaleExtensionsAndCopiesSelectedImageAsync()
    {
        ...
        await File.WriteAllTextAsync(staleImagePath, "old", TestContext.Current.CancellationToken);
        string sourceImagePath = Path.Combine(directory.Path, "selected.png");
        await File.WriteAllTextAsync(sourceImagePath, "new", TestContext.Current.CancellationToken);
        FileSystemModificationImageFileService service = CreateService(paths);
    ...
    public async Task ReplaceImageAsyncNoOpsWhenSourceAlready_IsDestinationAsync()
    {
        ...
        await File.WriteAllTextAsync(existingImagePath, "same", TestContext.Current.CancellationToken);
        FileSystemModificationImageFileService service = CreateService(paths);
        ...
        (await File.ReadAllTextAsync(existingImagePath, TestContext.Current.CancellationToken)).Should().Be("same");
    }
    ...
    public void CountImageFiles_RejectsImageDirectoryContainingALink()
    {
        ...
        Action act = () => service.CountImageFiles(ModificationType.Mod, "ShockWave");
