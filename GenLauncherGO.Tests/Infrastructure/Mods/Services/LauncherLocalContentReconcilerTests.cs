...
public sealed class LauncherLocalContentReconcilerTests
{
    ...
    /// <summary>
    ///     Snapshots outlive the content they describe because deletion never revisits the snapshot directory, so
    ///     reconciliation is what tells the integrity store which identifiers are still worth keeping.
    /// </summary>
    [Fact]
    public void Reconcile_RetainsIntegritySnapshotsForInstalledContentOnly()
    {
        using var directory = new TestDirectory();
        LauncherPaths paths = TestLauncherPaths.Create(directory.CreateDirectory("Game"));
        var localContentService = new RecordingLocalLauncherContentService();
        var integrityService = new RecordingContentIntegrityService();
        LauncherLocalContentReconciler reconciler = CreateReconciler(localContentService, integrityService);
        LauncherContentVersion installedVersion = TestLauncherContent.Version("Shockwave", "2.0", installed: true);
        LauncherContentVersion uninstalledVersion = TestLauncherContent.Version(
            "Shockwave",
            "1.0",
            sourceKind: ContentSourceKind.ManagedSingleFile);
        var launcherData = new LauncherData();
        launcherData.AddOrUpdate(installedVersion);
        launcherData.AddOrUpdate(uninstalledVersion);
        localContentService.InstalledVersions = [installedVersion];

        reconciler.Reconcile(launcherData, new List<LauncherContentKey>(), paths);

        integrityService.PrunedPaths.Should().ContainSingle().Which.Should().Be(paths);
        integrityService.RetainedTargetIdSets.Should().ContainSingle().Which.Should().BeEquivalentTo(
        [
            ContentIntegrityTargetId.ForPackage(installedVersion.ContentKey),
            ContentIntegrityTargetId.ForCache(installedVersion.ContentKey)
        ]);
    }
        ...
        IContentIntegrityService? integrityService = null)
    {
        return new LauncherLocalContentReconciler(
            localContentService,
            integrityService ?? new RecordingContentIntegrityService(),
