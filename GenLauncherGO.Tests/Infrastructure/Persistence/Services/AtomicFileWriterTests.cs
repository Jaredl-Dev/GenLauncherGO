...
public sealed class AtomicFileWriterTests
{
    ...
    [Fact]
    public async Task WriteFileIfMissingAsync_ExistingDestination_SkipsWriterAndKeepsContentsAsync()
    {
        using TestDirectory directory = new();
        string destinationPath = directory.CreateFile("asset.png", "existing asset");
        var writer = new AtomicFileWriter();
        bool writerCalled = false;
        ...
        writerCalled.Should().BeFalse();
        File.ReadAllText(destinationPath).Should().Be("existing asset");
    }
