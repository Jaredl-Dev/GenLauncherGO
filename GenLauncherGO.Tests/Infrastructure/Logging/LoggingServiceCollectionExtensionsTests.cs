...
public sealed partial class LoggingServiceCollectionExtensionsTests
{
    ...
    public void AddGenLauncherGoLogging_DiagnosticSettingControlsDebugEvents(
        bool enableDiagnosticLogging,
        bool expectDebugEvent)
    {
        using TestDirectory directory = new();
        string logDirectory = directory.GetPath("Logs");
        var services = new ServiceCollection();

        services.AddGenLauncherGoLogging(logDirectory, enableDiagnosticLogging);
        using (ServiceProvider provider = services.BuildServiceProvider())
        {
            provider
                .GetRequiredService<ILogger<LoggingServiceCollectionExtensionsTests>>()
                .LogDebug("Detailed diagnostic event");
        }

        string logText = File.ReadAllText(Directory.GetFiles(logDirectory, "GenLauncherGO-*.log").Single());
        logText.Contains("Detailed diagnostic event", StringComparison.Ordinal).Should().Be(expectDebugEvent);
    }
