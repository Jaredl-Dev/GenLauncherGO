...
public sealed class ResumableHttpFileDownloaderTests
{
    ...
    public async Task DownloadFileAsync_NormalizesDestinationBeforeCreatingItsFolderAsync()
    {
        ...
        (await File.ReadAllTextAsync(Path.Combine(testDirectory.Path, "nested", "mod.zip"), TestContext.Current.CancellationToken))
            .Should().Be("payload");
    ...
    public async Task DownloadFileAsync_ReturnsExistingCompleteFileWithoutRequestAsync()
    {
        using TestDirectory testDirectory = new();
        string destinationFilePath = Path.Combine(testDirectory.Path, "mod.big");
        await File.WriteAllTextAsync(destinationFilePath, "ready", TestContext.Current.CancellationToken);
        QueueHttpMessageHandler handler = new();
        ResumableHttpFileDownloader downloader = CreateDownloader(handler);
        RecordingProgress<DownloadProgress> progress = new();
    ...
    [Fact]
    public async Task DownloadFileAsync_MissingExpectedEmptyFile_CreatesItFromResponseAsync()
    {
        using TestDirectory testDirectory = new();
        string destinationFilePath = Path.Combine(testDirectory.Path, "placeholder.txt");
        QueueHttpMessageHandler handler = new();
        ...
        await downloader.DownloadFileAsync(
            new DownloadFileRequest(
                new Uri("https://example.test/placeholder.txt"),
                destinationFilePath,
                0),
            null,
            CancellationToken.None);
    ...
    [Fact]
    public async Task DownloadFileAsync_ExistingExpectedEmptyFile_ReturnsWithoutRequestAsync()
    {
        using TestDirectory testDirectory = new();
        string destinationFilePath = Path.Combine(testDirectory.Path, "placeholder.txt");
        await File.WriteAllBytesAsync(destinationFilePath, [], TestContext.Current.CancellationToken);
        QueueHttpMessageHandler handler = new();
        ResumableHttpFileDownloader downloader = CreateDownloader(handler);
    ...
    public async Task DownloadFileAsync_ExistingFileLargerThanExpected_RestartsFromZeroAsync()
    {
        using TestDirectory testDirectory = new();
        string destinationFilePath = Path.Combine(testDirectory.Path, "mod.big");
        await File.WriteAllTextAsync(destinationFilePath, "too-large", TestContext.Current.CancellationToken);
        QueueHttpMessageHandler handler = new();
    ...
    public async Task DownloadFileAsync_WhenResumeIsDisabled_ReplacesExistingFileAsync()
    {
        using TestDirectory testDirectory = new();
        string destinationFilePath = Path.Combine(testDirectory.Path, "mod.big");
        await File.WriteAllTextAsync(destinationFilePath, "stale", TestContext.Current.CancellationToken);
        QueueHttpMessageHandler handler = new();
        handler.Enqueue(_ => CreateResponse(HttpStatusCode.OK, Encoding.UTF8.GetBytes("fresh")));
        ResumableHttpFileDownloader downloader = CreateDownloader(handler);
