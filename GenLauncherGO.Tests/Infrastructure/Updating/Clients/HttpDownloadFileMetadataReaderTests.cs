...
public sealed class HttpDownloadFileMetadataReaderTests
{
    ...
    public async Task ReadMetadataAsync_UsesHeadContentDispositionFileNameStarAsync()
    {
        ...
        handler.Enqueue(_ => CreateResponse(
            HttpStatusCode.OK,
            "attachment; filename*=UTF-8''Folder%20Package.big",
            123));
        ...
        metadata.FileName.Should().Be("Folder Package.big");
    ...
    [InlineData("attachment; filename*=UTF-8''Folder%2FPackage.big")]
    [InlineData("attachment; filename=\"CON\"")]
    [InlineData("attachment; filename=\"Package.zip.\"")]
    public async Task ReadMetadataAsync_ThrowsWhenRemoteFileNameIsUnsafeAsync(string contentDisposition)
    {
        QueueHttpMessageHandler handler = new();
        handler.Enqueue(_ => CreateResponse(
            HttpStatusCode.OK,
