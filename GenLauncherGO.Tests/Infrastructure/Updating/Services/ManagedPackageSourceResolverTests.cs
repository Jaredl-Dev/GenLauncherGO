...
public sealed class ManagedPackageSourceResolverTests
{
        ...
        long? expectedTotalBytes)
    {
        ...
        ManagedPackageSourceResolver resolver = CreateResolver(metadataReader, manifestReader);
    ...
    [Fact]
    public async Task ResolveAsync_AfterSizeLookup_ReusesSingleFileMetadataAsync()
    {
        Uri downloadUri = new("https://example.test/contra.zip");
        var metadata = new DownloadFileMetadata(downloadUri, "contra.zip", 42);
        StubDownloadFileMetadataReader metadataReader = new((_, _) => Task.FromResult(metadata));
        ManagedPackageSourceResolver resolver = CreateResolver(
            metadataReader,
            new RecordingS3ObjectManifestReader());
        LauncherContentVersion version = TestLauncherContent.Version(
            "Contra",
            "009",
            simpleDownloadLink: downloadUri.ToString());

        await resolver.GetTotalBytesAsync(version, CancellationToken.None);
        ManagedPackageSourceResolver.PackageSource? source =
            await resolver.ResolveAsync(version, CancellationToken.None);

        source.Should().BeOfType<ManagedPackageSourceResolver.PackageSource.SingleFile>()
            .Which.Metadata.Should().BeSameAs(metadata);
        metadataReader.RequestCount.Should().Be(1);
    }

    [Fact]
    public async Task ResolveAsync_AfterSizeLookup_ReusesS3ManifestAsync()
    {
        RemoteFileManifestEntry[] files =
        [
            new("Data/one.big", "hash", 10)
        ];
        RecordingS3ObjectManifestReader manifestReader = new();
        manifestReader.Enqueue(files);
        ManagedPackageSourceResolver resolver = CreateResolver(
            new StubDownloadFileMetadataReader(),
            manifestReader);
        ...
        source.Should().BeOfType<ManagedPackageSourceResolver.PackageSource.S3>()
            .Which.Files.Should().Equal(files);
        manifestReader.Requests.Should().ContainSingle();
    }
    ...
    public async Task GetTotalBytesAsync_MapsMetadataFailureToUnavailableAsync()
    {
        ...
        ManagedPackageSourceResolver resolver = CreateResolver(metadataReader, new RecordingS3ObjectManifestReader());
        LauncherContentVersion version = TestLauncherContent.Version(
            "Contra",
            "009",
    ...
    public async Task GetTotalBytesAsync_ChangedS3FolderName_ResolvesTheRelocatedPackageAsync()
    {
        ...
        manifestReader.Enqueue(
            new RemoteFileManifestEntry("Data/one.big", "hash", 10),
            new RemoteFileManifestEntry("Data/two.big", "hash", 15));
        ManagedPackageSourceResolver resolver = CreateResolver(new StubDownloadFileMetadataReader(), manifestReader);
        LauncherContentVersion version = CreateS3Version("1.0");
    ...
    private static ManagedPackageSourceResolver CreateResolver(
        IDownloadFileMetadataReader metadataReader,
        IS3ObjectManifestReader manifestReader)
    {
        return new ManagedPackageSourceResolver(
            metadataReader,
            manifestReader,
