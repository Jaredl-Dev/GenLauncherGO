...
public sealed class S3PackageUpdaterBehaviorTests
{
    ...
    public async Task UpdateAsync_ReportsWholePackageBytesWhenLatestFilesAreReusedAsync()
    {
        ...
        Directory.CreateDirectory(Path.GetDirectoryName(latestFilePath)!);
        await File.WriteAllBytesAsync(latestFilePath, CreatePayload(660), TestContext.Current.CancellationToken);
    ...
    public async Task UpdateAsync_ReportsWholePackageBytesForPartialStagedDownloadAsync()
    {
        ...
        await File.WriteAllBytesAsync(partialFilePath, CreatePayload(5), TestContext.Current.CancellationToken);
    ...
    public async Task RepairFilesAsync_HashLetterCaseDiffers_SkipsDownloadAsync(bool manifestHashIsLowercase)
    {
        ...
        await File.WriteAllBytesAsync(installedFilePath, CreatePayload(5), TestContext.Current.CancellationToken);
        ...
        downloader.Requests.Should().BeEmpty();
        (await File.ReadAllBytesAsync(installedFilePath, TestContext.Current.CancellationToken)).Should().Equal(CreatePayload(5));
    }
        ...
        long expectedIntermediateBytes)
    {
        ...
        string destinationPath = Path.Combine(paths.InstalledPath.FullPath, "Data", "readme.txt");
        Directory.CreateDirectory(Path.GetDirectoryName(destinationPath)!);
        await File.WriteAllBytesAsync(destinationPath, CreatePayload(15), TestContext.Current.CancellationToken);
        RecordingFileDownloader downloader = new()
        {
    ...
    public async Task RepairFilesAsync_BigFileHashRetry_RemovesConvertedFailureBeforeRetryAsync()
    {
        ...
        await File.WriteAllBytesAsync(bigPath, CreatePayload(2), TestContext.Current.CancellationToken);
        ...
        (await File.ReadAllBytesAsync(gibPath, TestContext.Current.CancellationToken)).Should().Equal(CreatePayload(5));
    }
