...
public sealed class PackageDownloadServiceTests
{
    ...
    public async Task DownloadAsync_UsesLatestVersionLinkAndOwnedPathsForSingleFilePackageAsync()
    {
        ...
        (DownloadFileMetadata Metadata, PackageUpdatePathSet Paths) request =
            updater.Requests.Should().ContainSingle().Which;
        request.Metadata.DownloadUri.Should().Be(new Uri("https://www.dropbox.com/s/package/latest.zip?dl=1"));
    ...
    public async Task DownloadAsync_ReturnsCanceledWhenCancellationStopsPreCommitWorkAsync()
    {
        ...
        Task<PackageDownloadResult> download = service.DownloadAsync(
            ...
            null,
            cancellation.Token);
        await updater.Started.Task.WaitAsync(TestTimeouts.Wait, TestContext.Current.CancellationToken);
        cancellation.Cancel();
        PackageDownloadResult result = await download.WaitAsync(TestTimeouts.Wait, TestContext.Current.CancellationToken);
    ...
    public async Task DownloadAsync_RejectsSourcesThatAreNotLauncherManagedAsync(ContentSourceKind sourceKind)
    {
        using TestDirectory testDirectory = new();
        LauncherPaths paths = TestLauncherPaths.Create(testDirectory);
        LauncherContentVersion version = TestLauncherContent.Version(sourceKind: sourceKind);
        RecordingSingleFilePackageUpdater singleFileUpdater = new();
        RecordingS3PackageUpdater s3Updater = new();
        RecordingS3ObjectManifestReader manifestReader = new();
        PackageDownloadService service = CreateService(paths, singleFileUpdater, s3Updater, manifestReader);
        ...
        result.Status.Should().Be(PackageDownloadStatus.UnexpectedFailure);
        singleFileUpdater.Requests.Should().BeEmpty();
        manifestReader.Requests.Should().BeEmpty();
        s3Updater.UpdateRequests.Should().BeEmpty();
    }
